  ...
  data() {
    return {
      ...
      pikerVerifyStart(val) {
        ...
        // 五年前的时间不可选
        let yearLimit = ''
        const threeYearsAgo = dayjs().subtract(5, 'year').valueOf()
        return {
          disabledDate: (time) => {
            yearLimit = dayjs(time).valueOf() < threeYearsAgo
            if (val) {
              endLimit = dayjs(time).valueOf() > dayjs(val).valueOf()
      ...
      pikerVerifyEnd(val) {
        const threeYearsAgo = dayjs().subtract(3, 'year').valueOf()
        return {
          disabledDate: (time) => {
            if (val) {
              return dayjs(time).valueOf() < dayjs(val).valueOf()
            } else {
              return dayjs(time).valueOf() < threeYearsAgo
            }
          }
        }
      }
