<template>
  <el-scrollbar class="side-scroll-bar" :style="{height: scrollerHeight}">
    ...
    <el-tabs v-model="firstTab" class="no-bottom-margin indent-left-margin">
            ...
            <template #item="{ element: ctn }">
              <li class="container-widget-item" :title="ctn.displayName" @dblclick="addContainerByDbClick(ctn)">
                <span>
                  ...
                  {{i18n2t(`designer.widgetLabel.${ctn.type}`, `extension.widgetLabel.${ctn.type}`) || getDisplayName(ctn)}}
                </span>
              </li>
            </template>
          </draggable>
      ...
      <el-tab-pane v-if="showFormTemplates('assemblyComponentLib')" name="assemblyComponentLib">
        <template #label>
          <span><svg-icon icon-class="el-form-template" /> {{i18nt('designer.assemblyComponentLib')}}</span>
        </template>
        <el-collapse v-model="activeNames" class="widget-collapse">
          <el-collapse-item name="1" :title="i18nt('designer.containerTitle')">
            <draggable tag="ul" :list="assemblyComponent" item-key="key" :group="{name: 'dragGroup', pull: 'clone', put: false}"
                       :clone="handleContainerWidgetClone" ghost-class="ghost" :sort="false"
                       :move="checkContainerMove" @end="onContainerDragEnd">
              <template #item="{ element: ctn }">
                <li class="container-widget-item" :title="ctn.displayName" @dblclick="addContainerByDbClick(ctn)">
      ...
      <el-tab-pane v-if="false" name="formLib" style="padding: 8px">
        <template #label>
...
<script>
  import {containers as CONS, basicFields as BFS, advancedFields as AFS, customFields as CFS, assemblyComponent as ABC} from "./widgetsConfig"
  ...
  export default {
    ...
    methods: {
      getDisplayName(ctn) {
        return ctn.options.displayName
      },
      isBanned(wName) {
        return this.getBannedWidgets().indexOf(wName) > -1
      },

      showFormTemplates(key) {
        // 'formTemplates'
        if (this.designerConfig[key] === undefined) {
          return true
        }

        return !!this.designerConfig[key]
      },
      ...
      loadWidgets() {
        ...
        this.assemblyComponent = ABC.map(con => {
          return {
            key: generateId(),
            ...con,
            displayName: this.i18n2t(`designer.widgetLabel.${con.type}`, `extension.widgetLabel.${con.type}`)
